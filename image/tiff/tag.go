...
func DecodeTag(r ReadAtReader, order binary.ByteOrder) (*Tag, error) {
	...
	if valLen > 4 {
		...
		// 下标索引为0开始偏移 int64(t.ValOffset) 长度为int64(valLen)
...
func DecodeDefineTagData(r ReadAtReader, order binary.ByteOrder, fieldId uint16) (*Tag, error) {
	var tagId uint16
	// 2bit
	err := binary.Read(r, order, &tagId)
	if err != nil {
		return nil, errors.New("tiff: tag id read failed: " + err.Error())
	}
	// 标签ID 与 指定标签ID 不符, 直接返回
	if fieldId != tagId {
		return nil, nil
	}
	t := new(Tag)
