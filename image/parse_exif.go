...
func GetDefineTag(r io.Reader, name FieldName) (*tiff.Tag, error) {
	fieldId, ok := fieldSet[name]
	if !ok {
		return nil, fmt.Errorf("fieldName no exist: %s", name)
	}
	return tiff.DecodeDefineField(r, fieldId)
}
